...
function checkCanChi(game, seatData, targetPai) {
    ...
    // }
    // var count = seatData.countMap[targetPai];
    if (targetPai = 0 && targetPai >= 33) {//判断是否超过组合牌界限
        return;
    }
    var up = seatData.countMap[targetPai - 1];
    var down = seatData.countMap[targetPai + 1];
...
function checkCanTingPai(game, seatData) {
    ...
    //检查手上的牌是不是已打缺，如果未打缺，则不进行判定 2017-05-25 15:32:36 暂不使用
...
exports.chuPai = function (userId, pai) {
    ...
    for (var i = 0; i < game.gameSeats.length; ++i) {
        ...
        /*-----------------start 检查是否可以吃 2017-05-25 11:27:01---------------*/
